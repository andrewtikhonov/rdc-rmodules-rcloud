...
class RModulesJobService implements Job {
	...
	private void runRCommandList(stepList)
	{
		...
		stepList.each { String currentCommand ->
			...
			RObject rObject = c.parseAndEval("try("+reformattedCommand+",silent=TRUE)");
			...
			//if (r.inherits("try-error"))
			{
				...
				String rError = RCloudConnection.getString(rObject); 
				...
				if(rError ==~ /(?ms).*\|\|FRIENDLY\|\|.*/)
				{
					...
					newError = new RserveException(null,rError);
				}
				else
				{
					log.error("RserveException thrown executing job: " + rError)
					newError = new RserveException(null,"There was an error running the R script for your job. Please contact an administrator.");
				}
